...
class OrderService
{
    ...
    {        
        ...
        $warehouse= DB::selectOne("SELECT WhseLink, Code FROM ".env('SAGE_HOST_DB_NAME')." WhseMst WHERE Code=? ",[$order['warehouse_code']]);
        $lineInsertArray = [];
        foreach ($order['item_list'] as $line) {
            ...
            $fUnitPriceExcl = $line['item_price'] ?? $unitPrice->fExclPrice ?? 0;
            $fUnitPriceIncl = $line['item_price'] ?? $unitPrice->fInclPrice ?? 0;
            ...
            $isLineSaved = DB::insert(
                ...
                [
                    0, $invoiceID, 0, 0, $stk_items->Description_1, $stk_items->iUOMStockingUnitID, $stockId, $warehouse->WhseLink ?? 0, $stk_items->iUOMStockingUnitID, $unitPrice->iARPriceListNameID ?? '', $line['item_quantity'], $line['item_quantity'], $fUnitPriceExcl,
    ...
    {
        $agent=1;
        $sat_agent = DB::selectOne("SELECT idAgents, cAgentName FROM ". env('SAGE_HOST_DB_NAME')."_rtblAgents WHERE cAgentName='SAT'");
        if(!is_null($sat_agent)){
            $agent=$sat_agent->idAgents;
        }
        $existOnSage = DB::select("SELECT Autoindex  FROM " . env('SAGE_HOST_DB_NAME') . "[INVNUM] where ExtOrderNum='" . $order['transaction_id'] . "'");
        if (count($existOnSage) < 1) {
            $customer_details = $this->sageCustomerDetails($order['customer_code']);
            $orderDate = Carbon::today();
            DB::beginTransaction();
            $sageOrderNum='IOF'.$this->sageLastOrderNum();
            $header_result = DB::insert('INSERT INTO ' . env('SAGE_HOST_DB_NAME') . '[INVNUM] (DocType, OrderNum, ExtOrderNum, OrderDate, AccountID, iDCBranchID,[DocVersion],
                ...
                Address1, Address2, DocRepID, iINVNUMAgentID) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [
                4, $sageOrderNum, $order['transaction_id'], Carbon::parse($order['transaction_date'])->format('Y-m-d'), $customer_details->DCLink, 0, 3, 1, 0, 0, 0, $orderDate, $orderDate, $orderDate, 0, $customer_details->Name, 0, 'SOL Order',
            ...
            ,$this->salesRep($order['user_code']), $agent]);
            ...
            return ['status'=>false, 'message'=> 'Transaction '.$existOnSage[0]->AutoIndex.' already exist in sage.'];
        }
    }
    ...
    {
        $inv_recod= DB::selectOne("SELECT AutoIndex, ExtOrderNum FROM " . env('SAGE_HOST_DB_NAME') ."InvNum WHERE ExtOrderNum=?", [$orderNum]);
        return $inv_recod->AutoIndex ?? 0;
    }

    public function sageLastOrderNum(){
        try{
        $record=DB::selectOne("SELECT OrderNum, AutoIndex, DocType FROM ". env('SAGE_HOST_DB_NAME') ."InvNum With (TABLOCKX) WHERE OrderNum like 'IOF%' and DocType=4 order by AutoIndex desc");
        if(!is_null($record)){
            return substr($record->OrderNum,3)+1;
        }
        else return 1;
        }
        catch(\Exception $ex){
